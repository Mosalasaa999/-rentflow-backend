...
app.get('/', (req, res) => {
  res.status(200).json({ platform: 'RentFlow Engine', status: 'OPERATIONAL' });
});
...
// 1. Direct Twilio Test: prints the exact Twilio API response or error on your screen
app.get('/test-whatsapp', async (req, res) => {
  try {
    const twilioClient = twilio(env.TWILIO_ACCOUNT_SID, env.TWILIO_AUTH_TOKEN);
    const msg = await twilioClient.messages.create({
      from: `whatsapp:${env.TWILIO_WHATSAPP_FROM}`,
      to: 'whatsapp:+27781817491',
      body: '🏠 *RentFlow Live Test*\n\nWhatsApp integration is working! Your RentFlow engine is fully operational. 🚀'
    });
    res.status(200).json({
      status: 'DELIVERED',
      sid: msg.sid,
      to: msg.to,
      message: 'Message dispatched! Check your WhatsApp chat with Twilio.'
    });
  } catch (err: any) {
    res.status(500).json({
      status: 'TWILIO_ERROR',
...
// 2. Full Flow Test: auto-sets leases in Neon DB to today (26th) and triggers assessment
app.get('/test-cron', async (req, res) => {
  try {
    const today = new Date().getDate();
    const updated = await prisma.lease.updateMany({
      where: { status: 'ACTIVE' },
      data: { dueDayOfMonth: today }
    });
    
    await rentCronQueue.add('manual-trigger', { dryRun: false });
    res.status(200).json({
      status: 'SUCCESS',
      leasesUpdatedToToday: updated.count,
      message: `Active leases set to today (${today}th) and cron job queued! Check your WhatsApp!`
    });
  } catch (err: any) {
